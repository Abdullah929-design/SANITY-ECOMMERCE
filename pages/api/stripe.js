    ...
    try {
      ...
      const params = {
        ...
        line_items: req.body.map((item) => {
          let newImage = '/placeholder.jpg';
          try {
            const img = item.image[0].asset._ref;
            newImage = img.replace('image-', 'https://cdn.sanity.io/images/54y08kqm/production/').replace('-png', '.png');
          } catch (error) {
            console.error('Error generating image URL for Stripe:', error);
          }
